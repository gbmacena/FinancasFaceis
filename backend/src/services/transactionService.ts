...
import { CreateExpenseData, typeError } from "../types/index";
import { EntryData, UpdateExpenseData, CreateExpenseResponse } from "../types/index";

const prisma = new PrismaClient();

const transactionService = {
  createEntry: async (userId: string, data: EntryData) => {
    try {
  ...
  createExpense: async (userId: string, expenseData: CreateExpenseData): Promise<CreateExpenseResponse | void> => {
    const user = await prisma.user.findUnique({ where: { uuid: userId } });
  ...
  updateExpense: async (expenseUuid: string, updateData: UpdateExpenseData) => {
    try {
